...
def register(request):
    if request.method == 'POST':
        ...
        industory_id = request.POST.get('industory_id')
        email = request.POST.get('email')
        if not (username and password and industory_id and email):
            return render(request, 'register.html', {'error': '入力漏れ'})

        try:
            hashed_password = make_password(password)
            UserManager.create_user(username, hashed_password, industory_id, email)
